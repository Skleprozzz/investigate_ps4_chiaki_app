...
static MunitResult test_bright_ambassador(const MunitParameter params[], void *user)
{
    ...
    ps4app_rpcrypt_bright_ambassador(bright, ambassador, nonce, morning);

    munit_assert_memory_equal(PS4APP_KEY_BYTES, bright, bright_expected);
    munit_assert_memory_equal(PS4APP_KEY_BYTES, ambassador, ambassador_expected);
...
static MunitResult test_iv(const MunitParameter params[], void *user)
{
    ...
    ps4app_rpcrypt_init(&rpcrypt, nonce, morning);

    uint8_t iv[PS4APP_KEY_BYTES];

    err = ps4app_rpcrypt_generate_iv(&rpcrypt, iv, 0);
    if (err != PS4APP_ERR_SUCCESS)
        return MUNIT_ERROR;
    munit_assert_memory_equal(PS4APP_KEY_BYTES, iv, iv_a_expected);
    ...
    err = ps4app_rpcrypt_generate_iv(&rpcrypt, iv, 0x0102030405060708);
    if (err != PS4APP_ERR_SUCCESS)
        return MUNIT_ERROR;
    munit_assert_memory_equal(PS4APP_KEY_BYTES, iv, iv_b_expected);
